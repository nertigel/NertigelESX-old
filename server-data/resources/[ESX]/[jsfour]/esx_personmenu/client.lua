...
ESX.UI.Menu.Open(
	'default', GetCurrentResourceName(), 'id_card_menu',
	{
		title    = _U('dl_menu'),
		elements = {
			{label = _U('check_dl'), value = 'check'},
			{label = _U('show_dl'), value = 'show'}
		}
	},
	function(data2, menu2)
		if data2.current.value == 'check' then
...
ESX.UI.Menu.Open(
	'default', GetCurrentResourceName(), 'id_card_menu',
	{
		title    = _U('gl_menu'),
		elements = {
			{label = _U('check_gl'), value = 'check'},
			{label = _U('show_gl'), value = 'show'}
		}
	},
	function(data2, menu2)
		if data2.current.value == 'check' then
...
function OpenCivilianActionsMenu()
  ...
  'default', GetCurrentResourceName(), 'civilian_actions',
  {
    ...
    elements = {
	  ...
	  {label = _U('dl_card'), value = 'dl_kort'},
	  {label = _U('gl_card'), value = 'gl_kort'},	  
    ...
    function(data, menu)
	  ...
	  if data.current.value == 'dl_kort' then
      	OpenShowGiveDL()
      end
	  
	  if data.current.value == 'gl_kort' then
      	OpenShowGiveGL()
      end
              ...
              local playerPed = GetPlayerPed(-1)
                ...
                if ( IsPedSittingInAnyVehicle( playerPed ) ) then
                   if GetVehicleDoorAngleRatio(playerVeh, 5) > 0.0 then 
                      ...
                      TriggerEvent("pNotify:SendNotification", {text = _U('closed_trunk'), type = "error", queue = "vehiclemenu", timeout = 1000, layout = "bottomLeft"})
                    else
                      SetVehicleDoorOpen(playerVeh, 5, false)
                      frontleft = true
                      TriggerEvent("pNotify:SendNotification", {text = _U('opened_trunk'), type = "success", queue = "vehiclemenu", timeout = 1000, layout = "bottomLeft"})    
                   end
                end
            end
