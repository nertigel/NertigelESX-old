...
AddEventHandler('esx_vangelico_robbery:gioielli1', function()
	...
	if rndm == 1 then
		xPlayer.addInventoryItem('jewels', math.random(4, 12))
	end
	if rndm == 2 then
		xPlayer.addInventoryItem('rolex', math.random(3, 9))
	end
	if rndm == 3 then
		xPlayer.addInventoryItem('ring', math.random(10, 15))
	end
	if rndm == 4 then
		xPlayer.addInventoryItem('necklace', math.random(2, 4))
	end
	if rndm == 5 then
		xPlayer.addInventoryItem('diamond', math.random(1, 2))
	end
...
local function Craft(source)
	...
	SetTimeout(4000, function()
		...
		if PlayersCrafting[source] == true and CopsConnected >= Config.RequiredCopsSell then
			...
			local NecklaceQuantity = xPlayer.getInventoryItem('necklace').count
			local LaptopQuantity = xPlayer.getInventoryItem('laptop').count
